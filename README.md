...
| 922 Pallet Labeler | The second pass for the pallet labels. 922 Setup puts the PALLET 1/2/3 label on a card when it CREATES the card, so a batch whose cards went up before the pallets were assigned ends up with no labels - and re-running Setup would make duplicate cards, not labels. Point this at the batch folder once the Pallet & Rod Organizer is filled in and it labels the cards that are already on the board. It never creates a card, it leaves REPEAT and material labels alone, it is safe to run twice, and if the organizer is empty or unreadable it stops and tells you instead of doing half the job |
