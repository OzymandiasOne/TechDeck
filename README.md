...
Sheet Metal Calculators | GUI plugin - a library of shop calculators behind a picker, each defined by a simple field-list + formula so new ones drop in easily; first calculator is Flat Length (bend-allowance flat length from material K-factor, thickness, inside radius/ID/OD, and bend angle) 