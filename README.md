...
| 911 Inspection Dimensions | Reads every PART SKETCH drawing in a folder of nest package PDFs and writes out each part's dimensions to a text file, so the inspection sheet numbers don't have to be typed off the paper. Logs lengths, radii (R .40), chamfers, snipes, and the weld preps (the KB codes, with the side each one applies to); leaves out anything marked REF and anything that is part of a note on the drawing (both listed separately so you can see them). Also pulls the part number, work order, size and FAB DIM off the title block, and flags any drawing it could not read so nothing goes missing quietly |
