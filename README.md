...
| DXF Offset Tool | Applies a machining/coating allowance to plate DXFs: every hole grows by 1/16" diameter, the outer profile shrinks 1/16" per side, and hole geometry moves to a HOLES layer - each file is written back out as "name OFFSET.dxf" with everything else untouched. Amounts are adjustable in Settings; pick a folder of DXFs or set a single file |
