...
# ===========================================================================
# Generate Teams Cards -- delegated to the sibling 911 Teams Cards plugin
# ===========================================================================
# This whole engine (card work list, payload, posted-card ledger, and the
# EB 922 Schedule STATUS write-back) MOVED to plugins/911_teams_cards on
# 2026-08-10 when Teams cards became their own app, so nobody has to generate
# cards just to set up a batch (C.D. 2026-08-06). 911 Setup still offers the
# stage as an optional first step -- now defaulting OFF -- and still needs the
# write-back to mark a nest NEED MODEL once its batch is set up, so it imports
# the sibling rather than keeping a second copy. Same pattern as the
# 911_remove_ticket cover-stamp helpers above: one home, imported.
# ---------------------------------------------------------------------------
_teams_cards = None


def _load_teams_cards(log):
    """The sibling 911_teams_cards module, or None with one logged warning."""
    global _teams_cards
...
def _dialog_groups() -> list:
    ...
    It now defaults **OFF** (v2.0.0): the cards are their own app, **911 Teams
    Cards**, precisely so nobody has to make cards just to set up a batch
    (C.D. 2026-08-06). It stays here as an opt-in for the one-pass case, and
    since this dialog is now STICKY (``remember_as`` in run()), ticking it once
    is all it takes for someone who does want both every time.
...
def run(params: dict, progress_callback, cancel_event: threading.Event):
    ...
    # ------------------------------------------------------------------ #
    #
    # STICKY since v2.0.0 (A.T. 2026-08-07): the window reopens with whatever
    # you last ran, so a site that never wants Teams cards or the difficulty
    # label unticks them ONCE instead of on every single run before it can even
    # get to the batch prompt. Saved in settings.json, which app updates leave
    # alone; a toggle added by a later update still arrives at its own declared
    # default rather than inheriting one you were never shown.
    # ------------------------------------------------------------------ #
    stages = sdk.request_grouped_toggles(
        ...
        subtext=("Uncheck anything you do not want to run - your choices are "
                 "remembered for next time. Re-running a batch with only PDF "
                 "Stamping checked re-stamps the packets without touching the "
                 "nest workbooks."),
    ...
    # ------------------------------------------------------------------ #
    # Stage 0 -- Generate Teams Cards (v1.8.0; delegated since v2.0.0)
    #
    ...
    #
    # Defaults OFF now that it is also its own app (911 Teams Cards), which
    # owns the code; this runs the sibling's stage in-process so a one-pass
    # "cards then set up" run is still one run and one batch prompt.
    # ------------------------------------------------------------------ #
    batch_stages = do_folders or do_nest_data or do_inspection or do_stamping
    if do_cards:
        cards_mod = _load_teams_cards(log)
        if cards_mod is None:
            log("  Skipping Teams cards - run the '911 Teams Cards' app "
                "directly, or reinstall TechDeck to restore it.")
        else:
            cards_mod._run_teams_cards(params, progress_callback, cancel_event,
                                       qtdr_override,
                                       lo=0, hi=8 if batch_stages else 100)
    ...
    if do_folders and do_nest_data:
        ...
        if cards_mod is not None:
            try:
                _sched_template = cards_mod._load_card_template()
            except (OSError, json.JSONDecodeError) as exc:
                log(f"\n  ! EB 922 Schedule STATUS not updated - "
                    f"card_template.json could not be read ({exc}).")
        if cards_mod is not None and _sched_template is not None:
            cards_mod._advance_schedule_status(
                params, _sched_template, log,
                [(batch_number, n) for n in nest_numbers],
                [_sched_template.get("status_after_cards",
                                     cards_mod._STATUS_AFTER_CARDS_DEFAULT)],
                _sched_template.get("status_after_setup",
                                    cards_mod._STATUS_AFTER_SETUP_DEFAULT),
                cancel_event)
