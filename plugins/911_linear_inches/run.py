...
Closed-loop cut-length report for a 911 QTDR batch. The user points the plugin at a
batch's ``IGES FILES`` folder (or the batch folder). Each IGES subfolder is named by
the nest package number (``N<nest>S`` -> nest ``<nest>``) and holds that nest's part
DXFs. Cut length per part is computed EXACTLY from the DXF vector geometry; the per-part
quantity is read from the nest package PDF's "SUMMARY OF NEST" table (WORK ORDER -> qty),
matched to the DXF by the work order embedded in its filename. Output is one Excel report:
per-part rows, per-nest subtotals, and a batch total.

Part 2 (approximate nest-layout simulation for utilization/remnant data) is planned but
not built yet.
...
# ---------------------------------------------------------------------------
# Folder resolution + quantity lookup
# ---------------------------------------------------------------------------
...
def resolve_roots(selected):
    ...
    # Fall back to treating the picked folder as the IGES root.
    return selected, os.path.dirname(selected)
...
def load_nest_summary(nest, nest_pkg_dir, batch_dir, log):
    """Return {work_order: (part_number, qty)} for a nest, or {} if unavailable."""
    pdf = None
    if nest_pkg_dir:
    ...
    try:
        for page in doc:
            text = page.get_text()
            if "SUMMARY OF NEST" not in text:
                continue
...
def run(params, progress_callback, cancel_event):
    ...
    selected = sdk.request_directory(
        ...
        start_dir=settings.get("default_root", "") or "",
    )
    if not selected:
    ...
    for idx, sub in enumerate(subs):
        if cancel_event is not None and cancel_event.is_set():
            log("Cancelled.")
            return
        progress_callback(int(5 + 90 * idx / len(subs)))
        nest = derive_nest(sub)
        if nest == sub:
            flags.append(f"{sub}: folder name is not the expected N<nest>S format")
        summary = load_nest_summary(nest, nest_pkg_dir, batch_dir, log)
        if not summary:
        ...
        for dxf in dxfs:
            ...
            parts.append({"part": part, "wo": wo, "qty": qty,
    ...
    if flags:
        ...
        for f in flags:
            log(f"  ! {f}")
        sdk.show_warning(params, "Linear Inches - review needed",
