...
| `911_sspo_award_review` | 911 SSPO Award Review | 911 | (Renamed from "911 Runtime Estimator" 2026-07-13.) Prompts for a ROOT of order folders (an award package); estimates plate cut time from exact DXF linear inches, writes ONE workbook (Plates + Non-Plates sheets, each a data table + a **real** Excel PivotTable via COM), plus a Shape Ft Req sheet for receiving and a Working Forecast Input sheet (per-nest Source Material/Pieces/Orders from the packet) that pastes into the Working Forecast List |
